...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            help="Username assigned to the created geneset/versions."
        )

        parser.add_argument(
            '--gene_id',
            dest='gene_id',
            help=("Enter a gene identifier to use (other than Entrez, which " +
                  "is the default) to map genes."
            )
        )

        parser.add_argument(
            '--kegg_record_types',
            dest='kegg_record_types',
            help=("Desired KEGG Record Types, if something other than " +
                  "(Pathway, Module, Disease). Each record type should be " +
    ...
    def handle(self, *args, **options):
        ...
        try:
            org = Organism.objects.get(
                scientific_name = options.get('organism')
            )
        ...
        if version is None:
            logger.error(
                'The KEGG api may have changed. '
                'Release no longer starts with "Release".'
            )
        else:
        ...
        if (options.get('kegg_record_types')):
            kegg_record_types = (
                options.get('kegg_record_types')
            ).replace(" ", "").split(",")
            kegg_record_types = tuple(kegg_record_types)
            logger.info(
                'Requested KEGG Record Types are: %s',
                str(kegg_record_types)
            )
        ...
        for record_type in kegg_record_types:
            ...
            record_members = get_kegg_members(
                KEGG_URL_BASE, KEGG_NAMES[org.scientific_name], record_type
            )
            for (record, members) in record_members.iteritems():
                if record_type == 'Module':
                    # for modules, they are prefixed with species_
                    record = record.split('_').pop()
                slug = slugify(org.scientific_name + ' ' + record)
                gs_info = get_kegg_info(KEGG_URL_BASE, record)
                gs_info['title'] = (
                    'KEGG-' + record_type + '-' + record + ': ' + gs_info['title']
                )  # make title more search friendly
                try:
                ...
                if (options.get('gene_id')):
                    ...
                    if (gene_id == 'systematic_name'):
                        annots = frozenset(
                            [(gene.pk, None) for gene in Gene.objects.filter(systematic_name__in=members)]
                        )
                    elif (gene_id == 'standard_name'):
                        annots = frozenset(
                            [(gene.pk, None) for gene in Gene.objects.filter(standard_name__in=members)]
                        )
                    else:
                    ...
                    annots = frozenset(
                        [(gene.pk, None) for gene in Gene.objects.filter(entrezid__in=members)]
                    )

                most_recent_versions = Version.objects.filter(
                    geneset=geneset).order_by('-commit_date')[:1]
                description = ''
                ...
                if most_recent_versions:
                    ...
                    if (new or removed):
                        description = (
                            ...
                            ' annotations from KEGG version ' + version + '.'
                        )
                else:
                    description = (
