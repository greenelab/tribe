...
class Command(BaseCommand):
    option_list = BaseCommand.option_list + (
        ...
        make_option('--evidence', dest='evcodes', help="Evidence codes that are included. All others will be skipped. Comma separated (e.g. IEA,IDA)."),
        make_option('--user', dest='user', default="TribeUpdater", help="Username for the user to be assigned the created geneset/versions."),
        make_option('--obo', dest='obo', help="Location of obo file if not remote"),
        make_option('--annot', dest='annot', help="Location of annotation file if not remote"),
        make_option('--leading', dest='leading', action="store_true", help='Is there a leading tag on the database IDs column (i.e. the mouse gene ids all start with "MGI:" which must be removed).'),
        make_option('--remote', dest='remote', action="store_true", help='Load from remote URL. If used, this supercedes the annot and obo options.'),
        make_option('--initial', dest='initial', action="store_true", help='This is the initial load. If true, this uses dates from the annotation file to construct versions. Otherwise a difference from the current version is calculated to determine whether or not a new version should be made.'),
        make_option('--tair', dest='tair', action='store_true', help="When adding Arabidopsis GO terms, TAIR's locus IDs are not useful, so this command will find other gene IDs in GO file."),
        make_option('--only_wb', dest='only_wb', action='store_true', help="Load only GO terms that use WB identifiers."),
        make_option('--pseudomonas', dest='pseudomonas', action='store_true', help='If this option is passed, the command will search for gene systematic names as well as PseudoCAP identifiers.'),
    )

    help = 'Load GO annotations. If initial load, use dates for versions. Otherwise use changes from latest for versions.'
    def handle(self, *args, **options):
        ...
        loaded_obo = gene_ontology.load_obo(obo_location, remote_location=remote, timeout=5)

        if not loaded_obo:
            logger.error("Couldn't load OBO file %s with remote equal to %s.", obo_location, remote)
            sys.exit()

        annot_zip_fh = None
        annot_fh = None
        if remote:
            annot_zip_fh = urllib2.urlopen(GO_ASSOC_FTP + '.'.join((GO_ASSOC_PREFIX, GO_NAMES[org.scientific_name], GO_ASSOC_SUFFIX)), timeout=5)
        else:
        ...
        for line in annot_fh:
            ...
            (xrdb, xrid, details, goid, ref, ev, date) = (toks[0], toks[1], toks[3], toks[4], toks[5], toks[6], toks[13])
        ...
        if options.get('pseudomonas'):
            ...
            for (xrdb, xrids) in load_pairs.iteritems():
                ...
                logger.info("Mapped %s Pseudomonas genes from the database using gene systematic name.", gene_objs.count())
            ...
            for (xrdb, xrids) in load_pairs.iteritems():
                ...
                try:
                    ...
                    continue
                xrid_objs = CrossRef.objects.filter(crossrefdb=xrdb_obj).filter(xrid__in=xrids)
                logger.info("Mapped %s cross references from %s", xrid_objs.count(), xrdb)
                for xrid_obj in xrid_objs:
                    xref_cache[(xrdb, xrid_obj.xrid)] = xrid_obj.gene

        load_pmids(pubs)
        pub_cache = {}
        pub_values = Publication.objects.filter(pmid__in=pubs).only('id', 'pmid').values()
            ...
            for ref_item in refs:
                if ref_item.startswith('PMID:'):
                    try:
                        ...
                        pub = None
            gene_ontology.add_annotation(go_id=goid, gid=gene.pk, ref=pub, date=date, direct=True)
        ...
        for (term_id, term) in gene_ontology.go_terms.iteritems():
            if term.annotations:
                ...
                if options.get('initial'):
                    ...
                    logger.info('Initial load. Need to construct versions of %s from annotation date.', term.go_id)
                    date_annots = {}
                    for annotation in term.annotations:
                        date = timezone.make_aware(datetime.strptime(annotation.date, '%Y%m%d'), timezone.get_default_timezone())
                        try:
                    ...
                    for (date, annots) in sorted(date_annots.iteritems()):
                        annots_as_of_date.update([(annotation.gid, annotation.ref) for annotation in annots])
                        if (annots_as_of_date == prior_annots): #if nothing changed, continue
                            continue
                        v_obj = Version(geneset=gs_obj, creator=user, parent=prior_version, commit_date=date)
                        v_obj.description = "Added " + str(len(annots)) + " annotations from GO based on the dates provided in the GO annotation file."
                    ...
                    if most_recent_versions:
                        ...
                        if (new or removed):
                            description = description + 'Added ' + str(len(new)) + ' and removed ' + str(len(removed)) + ' annotations from GO.'
                    else:
                        description = 'Created with ' + str(len(annots)) + ' annotations from GO.'
                    if description:
