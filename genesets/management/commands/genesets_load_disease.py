...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        org = None
        try:   # Only exists for human
            org = Organism.objects.get(scientific_name='Homo sapiens')
        ...
        mim2gene_list = s.retr(
            ...
            auth=("anonymous", "casey.s.greene@dartmouth.edu")
        ).text.splitlines()
        ...
        for l in genemap_list:  # Loop from Dima @ Princeton
            # The choice of fields relies on info from the genemap.key file
            # from omim
            l_split = l.split('|')
        ...
        for (term_id, term) in disease_ontology.go_terms.iteritems():
            if term.annotations:
                ...
                description += (
                    ' Annotations from child terms in the disease ontology ' +
